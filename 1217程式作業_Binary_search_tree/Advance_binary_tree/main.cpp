...
class Mission{
  ...
  vector<tmpTree> treevec, hpvec, v2_treevec ;
  vector<string> deletevec ;
  int times = 0 ;
  TreePtr * tree = NULL ;
  TreePtr * v2_tree = NULL ;
  ...
  void Create(){
      ...
      while ( i < treevec.size() ){
          ...
          else if ( treevec[i].defense > temp->defense ){
              if ( temp->right == NULL ){
  ...
  void v2_Create(){
      ...
      TreePtr * temp = NULL ;
      int i = 0 ,high = 0 ;
      while ( i < v2_treevec.size() ){
          if ( v2_tree == NULL ){
              v2_tree = new TreePtr ;
              v2_tree->num = v2_treevec[i].num ;
              v2_tree->name = v2_treevec[i].name ;
              v2_tree->type = v2_treevec[i].type ;
              v2_tree->hp = v2_treevec[i].hp ;
              v2_tree->attack = v2_treevec[i].attack ;
              v2_tree->defense = v2_treevec[i].defense ;
              v2_tree->raw = v2_treevec[i].raw ;
          ...
          else if ( v2_treevec[i].defense >= temp->defense ){
              if ( temp->right == NULL ){
                  temp->right = new TreePtr ;

                  temp->right->num = v2_treevec[i].num ;
                  temp->right->name = v2_treevec[i].name ;
                  temp->right->type = v2_treevec[i].type ;
                  temp->right->hp = v2_treevec[i].hp ;
                  temp->right->attack = v2_treevec[i].attack ;
                  temp->right->defense = v2_treevec[i].defense ;
                  temp->right->raw = v2_treevec[i].raw ;
              ...
              if ( temp->left == NULL ){
                  ...
                  temp->left->num = v2_treevec[i].num ;
                  temp->left->name = v2_treevec[i].name ;
                  temp->left->type = v2_treevec[i].type ;
                  temp->left->hp = v2_treevec[i].hp ;
                  temp->left->attack = v2_treevec[i].attack ;
                  temp->left->defense = v2_treevec[i].defense ;
                  temp->left->raw = v2_treevec[i].raw ;
      ...
      int depth = treeHeight( v2_tree ) ;


      cout << "v2_tree high : " << depth+1 << "\n" ;
  }
  ...
  void Search( TreePtr * node, int sear ){
      ...
      if ( node->right == NULL && node->left == NULL && node->same == NULL ){
         ...
         return ;
          ...
          if ( node->defense < sear && node->right != NULL )
              Search( node->right, sear ) ;

          else if ( node->defense > sear ){
              tmpTree tempt ;
              tempt.num = node->num ;
              tempt.name = node->name ;
              tempt.hp = node->hp ;
  ...
  void Sort(){
      ...
      for (i = 0; i < hpvec.size() ; i++) {
          for (j = 0; j < hpvec.size() - i; j++){
              if (hpvec[j].defense < hpvec[j + 1].defense){
                  temptype = hpvec[j];
                  hpvec[j] = hpvec[j + 1];
                  hpvec[j + 1] = temptype;
              }
